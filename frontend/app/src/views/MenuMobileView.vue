<template>
  ...
  <div class="d-flex flex-column text-center">
    <ul class="navbar-nav bg-body-tertiary rounded shadow-sm mt-3">
      <!-- Analytics options based on user role -->
      <li class="nav-item" v-if="authStore.user?.role === 'coach'">
        <router-link :to="{ name: 'coach-analytics' }" class="nav-link link-body-emphasis w-100 py-3 fs-5">
          <font-awesome-icon :icon="['fas', 'fa-chart-bar']" />
          <span class="ms-1">Análisis</span>
        </router-link>
      </li>
      <li class="nav-item" v-if="authStore.user?.role === 'student'">
        <router-link :to="{ name: 'student-analytics' }" class="nav-link link-body-emphasis w-100 py-3 fs-5">
          <font-awesome-icon :icon="['fas', 'fa-chart-line']" />
          <span class="ms-1">Mi Progreso</span>
        </router-link>
      </li>
      <li class="nav-item">
